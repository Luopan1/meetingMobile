...
/**
 * Created by lichuanbei on 2016/10/16.
 *
 * 网络判断状态工具
 */
...
public class NetworkUtil {
    ...
    public static boolean printNetworkInfo(Context context) {
        ...
        if (connectivity != null) {
            ...
            L.i(TAG, "-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$-------------");
            L.i(TAG, "getActiveNetworkInfo: " + in);
            NetworkInfo[] info = connectivity.getAllNetworkInfo();
            if (info != null) {
                for (int i = 0; i < info.length; i++) {
                    // if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
                    L.i(TAG, "NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable());
                    L.i(TAG, "NetworkInfo[" + i + "]isConnected : " + info[i].isConnected());
                    L.i(TAG, "NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting());
                    L.i(TAG, "NetworkInfo[" + i + "]: " + info[i]);
                    // }
                }
                L.i(TAG, "\n");
            } else {
                L.i(TAG, "getAllNetworkInfo is null");
            }
        }
        return false;
    }
    ...
    public static int getConnectedTypeINT(Context context) {
        ...
        if (net != null) {
            //if (Log.isPrint) {
            L.i(TAG, "NetworkInfo: " + net.toString());
            //}
            return net.getType();
        }
        return -1;
