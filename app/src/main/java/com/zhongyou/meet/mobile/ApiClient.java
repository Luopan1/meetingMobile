...
public class ApiClient {
	 ...
	 * @param callback
	 */
	public void requestUser(Object tag, OkHttpCallback<com.alibaba.fastjson.JSONObject> callback) {
		okHttpUtil.get(Constant.getAPIHOSTURL() + "/osg/app/user", getCommonHead(), null, callback, tag);
	}


	public void requestUserOld(Object tag, OkHttpCallback<BaseBean<UserData>> callback) {
		okHttpUtil.get(Constant.getAPIHOSTURL() + "/osg/app/user", getCommonHead(), null, callback, tag);
	}
		...
		ApiClient.getInstance().requestUser(this, new OkHttpCallback<com.alibaba.fastjson.JSONObject>() {
			...
			public void onSuccess(com.alibaba.fastjson.JSONObject obj) {
				if (obj.getInteger("errcode") == 0) {
					UserData entity = JSON.parseObject(obj.getJSONObject("data").toJSONString(), UserData.class);
