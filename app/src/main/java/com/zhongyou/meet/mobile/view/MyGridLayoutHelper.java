 ...
 *  ---------------------
 */
public class MyGridLayoutHelper extends BaseLayoutHelper {
	...
	@NonNull
	private MyGridLayoutHelper.SpanSizeLookup mSpanSizeLookup = new MyGridLayoutHelper.DefaultSpanSizeLookup();
	 ...
	 */
	public MyGridLayoutHelper(int spanCount) {
		this(spanCount, -1, -1);
	}
	 ...
	 */
	public MyGridLayoutHelper(int spanCount, int itemCount) {
		this(spanCount, itemCount, 0);
	}

	public MyGridLayoutHelper(int spanCount, int itemCount, int gap) {
	 ...
	 */
	public MyGridLayoutHelper(int spanCount, int itemCount, int vGap, int hGap) {
	...
	public void setWeights(float[] weights) {
		...
		this.mWeights = new float[0];
		throw  new IllegalArgumentException("此模式不能传递weights 否则排列方式会错乱");

	}

	public void setSpanSizeLookup(MyGridLayoutHelper.SpanSizeLookup spanSizeLookup) {
	...
	public void layoutViews(RecyclerView.Recycler recycler, RecyclerView.State state, VirtualLayoutManager.LayoutStateWrapper layoutState, LayoutChunkResult result, LayoutManagerHelper helper) {
		...
		final boolean layoutStart = layoutState.getLayoutDirection() == VirtualLayoutManager.LayoutStateWrapper.LAYOUT_START;
			...
			if (layoutInVertical) {
				if (weighted) {
					...
					if (params.getViewPosition()<=2){
						...
						top=maxSize*params.getViewPosition()+index * mVGap;
						bottom = top + maxSize;
					}else if (params.getViewPosition()>=3&&params.getViewPosition()<=6){
						left=0;
						top=maxSize*(params.getViewPosition()-3)+index * mVGap;
						bottom=top + maxSize;
 ...
 */
	...
	private void assignSpans(RecyclerView.Recycler recycler, RecyclerView.State state, int count,
		...
		for (int i = start; i != end; i += diff) {
			...
			if (spanDiff == -1 && spanSize > 1) {
				...
				Log.e(TAG, "assignSpans:  "+i +"---"+mSpanIndices[i]);
			} else {
				mSpanIndices[i] = span;
	...
	static final class DefaultSpanSizeLookup extends MyGridLayoutHelper.SpanSizeLookup {
